...
window.addEventListener("load",()=>{
  ...
  if(searchInput)searchInput.addEventListener("input",renderFilteredCustomers);
  if(sortSelect)sortSelect.addEventListener("change",renderFilteredCustomers);
...
function renderCustomerCards(customers){
  ...
  if(!customers||customers.length===0){
    grid.innerHTML=`
      <div class="admin-empty-state">
        ...
        <p>Try changing your search or creating a customer from a measurement request.</p>
      </div>
    `;
    return;
  }
  ...
  customers.forEach(customer=>{
    ...
    card.onclick=()=>{
      window.location.href=`${ROUTES.CUSTOMER}?id=${customer.customer_id}`;
    };
